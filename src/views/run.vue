    ...
    export default {
        ...
        methods : {
            ...
            modelReset : function (myCanvas, i) {
                ...
                if((myCanvas.list[i].x > myCanvas.width - myCanvas.list[i].r || myCanvas.list[i].x < myCanvas.list[i].r)) {
                    myCanvas.list[i].vx = -myCanvas.list[i].vx;
                }

                if((myCanvas.list[i].y > myCanvas.height - myCanvas.list[i].r || myCanvas.list[i].y < myCanvas.list[i].r)) {
                    myCanvas.list[i].vy = -myCanvas.list[i].vy;
                }

                let vx = myCanvas.list[i].vx, vy = myCanvas.list[i].vy, rx = 0, ry = 0;
                for (let j= 0, len = myCanvas.list.length; j < len; j++) {
                    if(i != j) {
                        if(this.arcDistance(myCanvas.list[i].x, myCanvas.list[i].y, myCanvas.list[j].x, myCanvas.list[j].y) < (myCanvas.list[i].r + myCanvas.list[j].r)) {
                            vx = myCanvas.list[i].vx;
                            vy = myCanvas.list[i].vy;
                            rx = myCanvas.list[j].vx;
                            ry = myCanvas.list[j].vy;

                            myCanvas.list[i].vx = rx;
                            myCanvas.list[i].vy = ry;
                            myCanvas.list[j].vx = vx;
                            myCanvas.list[j].vy = vy;
                        }
                    }
                }
            },
            arcDistance : function (x1, y1, x2, y2) {
